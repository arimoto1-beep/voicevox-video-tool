...
- `--ass-font-size` / `--ass-margin-v` が指定された場合だけASS字幕スタイルへ一時反映する。
- `--ass-wrap-chars` が指定された場合だけASS本文を文字数ベースで簡易折り返しする。
- SRTファイル自体は変更しない。
...
| --- | --- | --- | --- | --- |
| ASS-STYLE-001 | `get_ass_subtitle_style` | `short` layout用スタイルが期待値になること | `test_get_ass_subtitle_style_returns_short_style` | font_name Yu Gothic UI, font_size 96, margin_v 260 |
| ASS-STYLE-002 | `get_ass_subtitle_style` | `normal` layout用スタイルが期待値になること | `test_get_ass_subtitle_style_returns_normal_style` | font_name Yu Gothic UI, font_size 72, margin_v 150 |
| ASS-STYLE-003 | `apply_ass_style_overrides` | 未指定時に元のstyleと同じ値になること | `test_apply_ass_style_overrides_keeps_style_when_values_are_not_given` | デフォルト維持 |
| ASS-STYLE-004 | `apply_ass_style_overrides` | `font_size` だけ上書きできること | `test_apply_ass_style_overrides_replaces_font_size_only` | CLI試作用 |
| ASS-STYLE-005 | `apply_ass_style_overrides` | `margin_v` だけ上書きできること | `test_apply_ass_style_overrides_replaces_margin_v_only` | CLI試作用 |
| ASS-STYLE-006 | `apply_ass_style_overrides` | `font_size` と `margin_v` を同時に上書きできること | `test_apply_ass_style_overrides_replaces_font_size_and_margin_v` | CLI試作用 |
| ASS-STYLE-007 | `apply_ass_style_overrides` | 元のstyleが変更されないこと | `test_apply_ass_style_overrides_does_not_mutate_original_style` | frozen style維持 |
| ASS-STYLE-008 | `apply_ass_style_overrides` | 不正な `font_size` / `margin_v` で `ValueError` になること | `test_apply_ass_style_overrides_raises_for_invalid_font_size`, `test_apply_ass_style_overrides_raises_for_invalid_margin_v` | 値チェック |
...
| ASS-WRAP-001 | `wrap_subtitle_text` | `wrap_chars=None` では元の文字列を返すこと | `test_wrap_subtitle_text_returns_original_when_wrap_chars_is_none` | 未指定時は従来挙動 |
| ASS-WRAP-002 | `wrap_subtitle_text` | 空文字や短い文字列を壊さないこと | `test_wrap_subtitle_text_keeps_empty_text`, `test_wrap_subtitle_text_does_not_wrap_short_text` | 簡易折り返し |
| ASS-WRAP-003 | `wrap_subtitle_text` | `wrap_chars` 指定時に `\N` が入ること | `test_wrap_subtitle_text_inserts_ass_newline_when_wrap_chars_is_given` | ASS改行 |
| ASS-WRAP-004 | `wrap_subtitle_text` | `max_lines` を超える分は最後の行にまとまること | `test_wrap_subtitle_text_keeps_max_two_lines`, `test_wrap_subtitle_text_merges_remaining_text_into_last_line` | 省略はしない |
| ASS-WRAP-005 | `wrap_subtitle_text` | 既存改行がASS改行として扱われること | `test_wrap_subtitle_text_treats_existing_newline_as_ass_newline` | SRT複数行由来 |
| ASS-WRAP-006 | `wrap_subtitle_text` | 不正な `wrap_chars` / `max_lines` で `ValueError` になること | `test_wrap_subtitle_text_raises_for_invalid_wrap_chars`, `test_wrap_subtitle_text_raises_for_invalid_max_lines` | 値チェック |
| ASS-WRAP-007 | `escape_ass_text` | 折り返しで入れた `\N` が保持され、波括弧エスケープも壊れないこと | `test_escape_ass_text_keeps_wrapped_ass_newline_and_escapes_braces` | エスケープ順序 |
...
| ASS-CONTENT-003 | `build_ass_content` | Style行にstyle値が反映されること | `test_build_ass_content_builds_ass_sections_and_dialogues`, `test_build_ass_content_uses_normal_style_values` | font_name, font_size, margin_v, outline, shadow |
...
| ASS-CONTENT-005 | `build_ass_content` | `wrap_chars` 指定時にDialogue本文へ `\N` が入ること | `test_build_ass_content_wraps_dialogue_text_when_wrap_chars_is_given` | ASS本文だけ |
| ASS-CONTENT-006 | `build_ass_content` | `wrap_chars` 未指定時は従来どおり折り返さないこと | `test_build_ass_content_does_not_wrap_dialogue_text_when_wrap_chars_is_none` | 既存挙動維持 |
...
| ASS-ARGS-003 | `parse_args` | `--ass-font-size` / `--ass-margin-v` 指定時に `VideoOptions` に入ること | `test_parse_args_accepts_ass_style_overrides` | 字幕スタイル試作用 |
| ASS-ARGS-004 | `parse_args` | `--ass-wrap-chars` / `--ass-max-lines` 指定時に `VideoOptions` に入ること | `test_parse_args_accepts_ass_wrap_options` | 簡易折り返し |
...
| ASS-GEN-005 | `generate_video` | `ass_font_size` / `ass_margin_v` 指定時に生成ASSのStyle行へ反映されること | `test_generate_video_with_srt_applies_ass_style_overrides` | ffmpeg本体は起動しない |
| ASS-GEN-006 | `generate_video` | `ass_wrap_chars` 指定時に生成ASSのDialogue行へ `\N` が反映されること | `test_generate_video_with_srt_applies_ass_wrap_options` | ffmpeg本体は起動しない |
...
- `-vf` へのASS filter連結
- `--srt` のCLI引数パース
- `--ass-font-size` / `--ass-margin-v` のCLI引数パース
- `--ass-wrap-chars` / `--ass-max-lines` のCLI引数パース
